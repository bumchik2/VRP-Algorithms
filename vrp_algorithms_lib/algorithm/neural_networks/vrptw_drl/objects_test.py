...
from vrp_algorithms_lib.algorithm.neural_networks.vrptw_drl.objects import ProblemState,\
    Action, DepotId, CourierId, LocationId, extract_routes_from_problem_state, Routes, Route
import numpy as np


def test_problem_state_init(problem_state_for_tests: ProblemState):
...
def test_update(problem_state_for_tests: ProblemState):
    ...
    depot_idx = len(problem_state_for_tests.problem_description.locations)
    location_1_idx = problem_state_for_tests.location_id_to_idx[LocationId('location_1')]
    assert problem_state_for_tests.locations_idx == [[depot_idx, location_1_idx],
                                                     [depot_idx, depot_idx]]


def test_extract_routes_from_problem_state(problem_state_for_tests: ProblemState):
    action_1 = Action(location_id=LocationId('location_1'), courier_id=CourierId('courier_1'))
    problem_state_for_tests.update(action_1)
...
def test_reward(problem_state_for_tests: ProblemState):
    action_1 = Action(location_id=LocationId('location_1'), courier_id=CourierId('courier_1'))
    assert problem_state_for_tests.get_reward(action=action_1, routes=None) == -8 * problem_state_for_tests.problem_description.\
    ...
    assert np.isclose(problem_state_for_tests.get_reward(action=action_2, routes=None), expected_reward)


def test_reward_for_potential_latenesses(problem_state_for_tests: ProblemState):
    problem_state_for_tests: ProblemState
    action_1 = Action(location_id=LocationId('location_1'), courier_id=CourierId('courier_1'))
    routes = Routes(
        routes=[
            Route(
                vehicle_id=CourierId('courier_1'),
                location_ids=[
    ...
    actual_reward = problem_state_for_tests.get_reward(action=action_1, routes=routes)
    expected_reward = -8 * problem_state_for_tests.problem_description. \
        distance_matrix.depots_to_locations_distances[DepotId('depot_1')][LocationId('location_1')] - 3.0 * 1
