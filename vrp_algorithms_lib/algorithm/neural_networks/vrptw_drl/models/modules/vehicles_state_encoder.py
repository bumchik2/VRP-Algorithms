...
class VehiclesStateEncoder(nn.Module):
    def __init__(
            ...
            vehicles_state_hidden_dim: int,  # 512 in the original article
    ):
        """
        ...
        :param vehicles_state_hidden_dim: hidden dimension
        """

        super().__init__()

        self.fc = nn.Linear(vehicles_state_information_dim, vehicles_state_hidden_dim)
    ...
    def forward(
        ...
        hidden = nn.ReLU()(self.fc(vehicles_state_information))  # vehicles_number x vehicles_state_hidden_dim
        return hidden
